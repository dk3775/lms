...
if (isset($_POST['login'])) {
	...
	if ($na2 == "IN") {
		...
		if ($row['InstituteUserName'] == $u and password_verify($pass, $row['InstitutePassword'])) {
			session_start();
			$_SESSION['id'] = $u;
			$_SESSION['role'] = "Texas";
		...
		$result = mysqli_query($conn, $sql);
		$row = mysqli_fetch_assoc($result);
		if ($row['FacultyUserName'] == $u and password_verify($pass, $row['FacultyPassword'])) {
			session_start();
			$_SESSION['id'] = $u;
			$_SESSION['role'] = "Lagos";
		...
		$result = mysqli_query($conn, $sql);
		$row = mysqli_fetch_assoc($result);
		if ($row['StudentUserName'] == $u and $row['StudentPassword'] == $pass) {
			session_start();
